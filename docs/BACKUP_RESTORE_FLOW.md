# HOMIQ Backup and Restore

Phase 8 implements portable, local-first backup and restore.

## Backup format

HOMIQ exports a versioned JSON document.

Default file name:

`HOMIQ-backup-YYYY-MM-DD-HHmm.homiq.json`
...
HOMIQ deliberately does not copy the raw SQLite database file.
...
HOMIQ never receives the Google password.
...
Before the confirmation dialog appears, HOMIQ:

1. Opens the selected document.
2. Verifies the HOMIQ backup marker.
...
HOMIQ stores local timestamps for:
...
Phase 8 uses Android's system document picker rather than a HOMIQ cloud server.
...
- A HOMIQ-hosted backend
- Server subscription costs
- Drive passwords inside HOMIQ
